<script lang="ts">
	...
	import { productIdsToDataset } from '$lib/utils/helpers';
	import { Kaljakori } from '$lib/alko';
	import { getItemQuantity, getListById, listToURI, removeFromList, saveList } from '$lib/utils/lists';
...
<div class="flex flex-row items-center justify-between border-b border-gray-300 bg-gray-50 p-3 md:p-4">
	{#if existingList}
		<input type="text" onblur={() => validateListName(list.name)} class="border-none p-0 text-2xl bg-transparent w-full focus:ring-0 leading-none" bind:value={list.name} />
	{:else}
		<h2 class="text-lg md:text-2xl leading-none">{list.name}</h2>
		<button
			class={twMerge(components.button({ type: 'positive', size: 'md' }), 'ml-4')}
			onclick={() => saveList(list)}
		>
		<span>Tallenna lista</span> <Icon name="save" />
		</button>
	{/if}
</div>
<div class={twMerge("relative grid h-full", kaljakori.data.length > 1 && "grid-cols-[auto_1fr]")}>
	{#if kaljakori.data.length === 0}
		<div class="flex-auto grid place-content-center">
			<div class="prose text-center mx-auto">  
				<h2>Lista on tyhjä!</h2>
				<p>Lisää tuotteita listaan tuotevalikosta!</p>
    			<a href="/" class={twMerge(components.button({type: "positive"}), "mx-auto w-full")}><span>Tuotevalikkoon</span><Icon name="arrow_right" /></a>
			</div>
		</div>
	{:else}
									...
									<div class="flex w-full flex-col gap-2">
										...
										<div class="flex flex-col gap-4 md:flex-row md:items-center">
											<div class="flex items-center gap-2">
												...
												<span class="text-sm text-gray-500">{getItemQuantity(list, item[AllColumns.Number])} kpl</span>
											</div>
